-- ============================================================================
-- GMV On-Line (vendas do site, tipo_pdv='E') + corte por mês (comparativo)
-- ----------------------------------------------------------------------------
-- - bi_group / bi_events: nova coluna gmv_online (somatório de gmv só do Site).
-- - bi_group / bi_summary: p_month_max -> limita aos meses <= valor (para
--   comparar com o ano anterior só até o último mês com vendas).
-- ============================================================================

-- bi_group: + gmv_online, + p_month_max.
drop function if exists bi_group(uuid, int, text, text[], text);
create or replace function bi_group(
  p_org uuid, p_year int, p_datebase text, p_pdv text[], p_dim text,
  p_month_max int default null
)
returns table(
...
-- bi_events: + gmv_online no retorno (mesmos parâmetros).
...
returns table(
  ...
  mdr numeric, rebate numeric, total_count bigint
)
language sql stable security definer set search_path = public
as $$
  with agg as (
  ...
  select codigo_evento, nome, segmento, genero, organizador, local, cidade, uf,
    data_evento, qtd, gmv, coalesce(gmv_online,0), receita_bt, receita_liq, mdr, rebate,
...
-- bi_summary: + p_month_max (mantém o retorno).
drop function if exists bi_summary(uuid, int, text, text[]);
