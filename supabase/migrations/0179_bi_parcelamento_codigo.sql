-- bi_parcelamento retorna também o codigo_evento (para link ao painel
-- Blueticket nas linhas de evento). Muda o retorno -> drop necessário.
drop function if exists bi_parcelamento(uuid, int, text[], text, int[], int, text, text);
...
as $$
  ...
  select
    ...
    case when p_dim = 'evento' then gk else null end as codigo,
