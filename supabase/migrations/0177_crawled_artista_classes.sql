-- "Com Artista" vira filtro por CLASSE de artista (A+/A/B/C, múltiplas).
-- Filtra os locais/organizadores que têm artistas das classes selecionadas e
-- retorna a contagem de artistas distintos encontrados (coluna `artistas`).
-- Troca p_com_artista (boolean) por p_classes (text[]) -> drop necessário.
drop function if exists crawled_organizers(text, numeric, text, text, text, boolean);
drop function if exists crawled_locals(text, numeric, text, text, text, boolean);
...
returns table (
  chave text, nome text, eventos bigint, artistas bigint,
...
as $$
  with ev as (
    select e.id, e.organizador_raw, e.cidade, e.uf, e.preco_min, e.preco_max,
           e.taxa_pct, e.data_inicio,
           lower(trim(e.organizador_raw)) as chave, cs.nome as fonte_nome
    from crawled_events e
    left join crawler_sources cs on cs.id = e.source_id
    where coalesce(e.ignorado, false) = false
      and e.organizador_raw is not null and trim(e.organizador_raw) <> ''
  ...
  agg as (
    select
      ev.chave,
      min(ev.organizador_raw) as nome,
      ...
      coalesce(array_agg(distinct (ev.cidade || case when ev.uf is not null and ev.uf <> '' then '/' || ev.uf else '' end))
               filter (where ev.cidade is not null and ev.cidade <> ''), '{}') as cidades,
      coalesce(array_agg(distinct ev.fonte_nome) filter (where ev.fonte_nome is not null), '{}') as fontes,
  ...
  select agg.chave, agg.nome, agg.eventos, coalesce(art.n, 0)::bigint as artistas,
    agg.preco_min, agg.preco_max, agg.taxa_media,
    agg.cidades, agg.fontes, agg.cidade_nome, agg.uf, agg.proximo
  from agg
...
as $$
  with ev as (
    select e.id, e.local_raw, e.cidade, e.uf, e.preco_min, e.preco_max,
           ...
           lower(trim(e.local_raw)) || '|' || coalesce(
             case when e.cidade is null or e.cidade = '' then null
                  else e.cidade || case when e.uf is not null and e.uf <> '' then '/' || e.uf else '' end end, '') as chave,
           case when e.cidade is null or e.cidade = '' then null
                else e.cidade || case when e.uf is not null and e.uf <> '' then '/' || e.uf else '' end end as cidade_uf
    from crawled_events e
    left join crawler_sources cs on cs.id = e.source_id
    where coalesce(e.ignorado, false) = false
      and e.local_raw is not null and trim(e.local_raw) <> ''
...
grant execute on function crawled_organizers(text, numeric, text, text, text, text[]) to authenticated;
